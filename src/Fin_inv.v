...
(* This is the most powerful induction scheme*)
Definition Fin_t_S_inv n i :
  ∀ (P : t (S n) → Type), P F1 → (∀i, P (FS i)) → P i.
Proof.
  refine 
  (match i as i' in Fin.t n' return 
    (match n' as n'' return (Fin.t n'' → Type) with
    | 0 => fun _ => IDProp
    | S n'' => fun (ea : Fin.t (S n'')) =>
       ∀ (P : t (S n'') → Type), P F1 → 
       (∀ (eb : Fin.t n''), P (FS eb)) → P ea 
    end i')
  with
  | F1 => _ 
  | FS j => _ 
...
(* this can proven from the top one Fin_t_S_inv *)
...
(* This can be proven from Fin_t_S_inv_convoy *)
Lemma Fin_t_S_inv_gen_convoy : ∀ (P : ∀ (n : nat), t (S n) → Type),
   (∀ (n : nat), P n F1) → 
...
(* Below code will not go through *)
